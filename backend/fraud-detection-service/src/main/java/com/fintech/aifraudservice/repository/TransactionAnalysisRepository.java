...
public interface TransactionAnalysisRepository extends JpaRepository<TransactionAnalysis, Long> {
  ...
  Page<TransactionAnalysis> findByRiskLevel(
      TransactionAnalysis.RiskLevel riskLevel, Pageable pageable);

  Page<TransactionAnalysis> findByFraudStatus(
      TransactionAnalysis.FraudStatus fraudStatus, Pageable pageable);

  @Query(
      "SELECT ta FROM TransactionAnalysis ta WHERE ta.userId = :userId AND ta.createdAt >= :since")
  List<TransactionAnalysis> findUserTransactionsSince(
  ...
  @Query(
      "SELECT ta FROM TransactionAnalysis ta WHERE ta.userId = :userId ORDER BY ta.createdAt DESC")
  List<TransactionAnalysis> findRecentUserTransactions(
