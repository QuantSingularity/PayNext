...
async def lifespan(app: FastAPI):
    ...
    try:
        credit_scoring_model = joblib.load(
            os.path.join(_SERVICE_DIR, "credit_scoring_model.joblib")
        )
        credit_scoring_scaler = joblib.load(
            os.path.join(_SERVICE_DIR, "credit_scoring_scaler.joblib")
        )
        credit_scoring_features = joblib.load(
            os.path.join(_SERVICE_DIR, "credit_scoring_features.joblib")
        )
        logger.info("Credit Scoring Model, Scaler, and Features loaded successfully.")
    except FileNotFoundError:
        logger.error(
            f"Credit scoring model, scaler, or features file not found at {_SERVICE_DIR}. "
            "Please train the model first using credit_scoring_model.py."
        )
