...
async def lifespan(app: FastAPI):
    ...
    try:
        churn_model = joblib.load(os.path.join(_SERVICE_DIR, "churn_model.joblib"))
        churn_scaler = joblib.load(os.path.join(_SERVICE_DIR, "churn_scaler.joblib"))
        churn_model_features = joblib.load(
            os.path.join(_SERVICE_DIR, "churn_model_features.joblib")
        )
        logger.info("Churn Prediction Model, Scaler, and Features loaded successfully.")
    except FileNotFoundError:
        logger.error(
            f"Churn model, scaler, or features file not found at {_SERVICE_DIR}. "
            "Please train the model first using churn_prediction_model.py."
        )
...
if __name__ == "__main__":
    ...
    uvicorn.run("churn_prediction_api:app", host="0.0.0.0", port=9003, reload=False)
