...
public interface AuditLogRepository extends JpaRepository<AuditLog, Long> {
  ...
  @Query(
      "SELECT al FROM AuditLog al WHERE al.userId = :userId AND al.createdAt BETWEEN :startDate AND"
          + " :endDate")
  ...
  @Query(
      "SELECT al.ipAddress, COUNT(al) as count FROM AuditLog al WHERE al.createdAt > :since GROUP"
          + " BY al.ipAddress ORDER BY count DESC")
  List<Object[]> findTopIPAddresses(@Param("since") LocalDateTime since);

  /**
   * Bulk delete audit logs older than the given cutoff — avoids loading all rows into memory.
   */
  @Modifying
  @Query("DELETE FROM AuditLog al WHERE al.createdAt < :cutoff")
